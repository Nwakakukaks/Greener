...
import "./GreenerFactory.sol";
import "./RateConverter.sol";

error Greener__not_implemented();
error Greener__after_endDate();
...
contract Greener {
    ...
    GreenerModel.FundraiserType public immutable i_type;
    GreenerModel.FundraiserCategory public immutable i_category;
    uint public immutable i_endDate;
    uint public immutable i_goalAmount;
    GreenerFactory private immutable i_factory;
    ...
    GreenerModel.FundraiserStatus public s_status;
    ...
    constructor(
        ...
        i_factory = GreenerFactory(msg.sender);
        i_endDate = _endDate;
        i_goalAmount = _goalAmount; // in USD cents! 1 USD = 100 _goalAmount
        s_status = GreenerModel.FundraiserStatus.ACTIVE;
    }
    ...
    function makeDonation(address _donorAddress, uint _amount, address _tokenAddress) external payable returns (bool) {
        require(s_status == GreenerModel.FundraiserStatus.ACTIVE, "You cannot donate to a fundraiser that is not active");
        require(_amount > 0, "Cannot deposit 0");
        if (i_endDate > 0 && block.timestamp > i_endDate) {
            revert Greener__after_endDate();
        }
        ...
        if (_tokenAddress == address(0)) {
            ...
            // TODO
            revert Greener__not_implemented();
        }
    ...
    function closeAndRevertDonations() public onlyOwner {
        require(s_status == GreenerModel.FundraiserStatus.ACTIVE, "You can only close active fundraisers");
        // TODO return donations
        s_status = GreenerModel.FundraiserStatus.CLOSED;
    }
    ...
    function getAllDetails() public view returns (
        uint id,
        address owner,
        GreenerModel.FundraiserType fType,
        GreenerModel.FundraiserCategory category,
        uint endDate,
        uint goalAmount,
        ...
        uint defaultImage,
        string memory name,
        GreenerModel.FundraiserStatus status,
        uint balances
    ) {
        return (
            i_id,
    ...
    function finalizeDonation() internal {
        if (i_goalAmount > 0) {
            ...
            if (totalDonationsInCents >= i_goalAmount) {
                s_status = GreenerModel.FundraiserStatus.FULLY_FUNDED;
            }
        }
    }
