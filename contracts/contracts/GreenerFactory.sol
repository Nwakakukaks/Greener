...
import "./GreenerModel.sol";
import "./Greener.sol";

    error GreenerFactory__deposit__zero_deposit();
    error GreenerFactory__deposit__less_than_declared();
    error GreenerFactory__deposit__token_not_allowed();
    error GreenerFactory__recurring__only_owner();
    error GreenerFactory__not_implemented();

contract GreenerFactory is KeeperCompatibleInterface, Ownable {
    /* State variables */
    ...
    mapping(uint => GreenerModel.RecurringPaymentDisposition) public s_recurringPayments;
    mapping(address => uint[]) public s_recurringPaymentsByOwner;
    AggregatorV3Interface public s_priceFeed;
    uint i_recurringInterval = 1 hours;

    /* Events */
    event FundraiserCreated(uint indexed fundraiserId, address indexed creator, string title, GreenerModel.FundraiserType fundraiserType, GreenerModel.FundraiserCategory category, uint endDate, uint goalAmount);
    ...
    function depositFunds(uint _amount, address _tokenAddress) external payable {
        if (_amount == 0) {
            revert GreenerFactory__deposit__zero_deposit();
        }
        uint currentBalance = s_userBalances[msg.sender][_tokenAddress];
        if (_tokenAddress == address(0)) {
            // ETH deposit
            if (msg.value < _amount) {
                revert GreenerFactory__deposit__less_than_declared();
            }
            ...
            if (!isTokenAllowed(_tokenAddress)) {
                revert GreenerFactory__deposit__token_not_allowed();
            }

            /// TODO
            revert GreenerFactory__not_implemented();
        }
    ...
    function createFundraiser(
        GreenerModel.FundraiserType _type,
        GreenerModel.FundraiserCategory _category,
        ...
        Greener fundraiser = new Greener(
            fundraiserId,
            msg.sender,
            _type,
            _category,
    ...
    function listFundraisersByStatus (GreenerModel.FundraiserStatus _status) public view returns (GreenerModel.FundraiserItem[] memory fundraisersByStatus) {
        uint i_totalFundraisersWithStatus = 0;

        for(uint i = 0; i < s_counterFundraisers; i++) {
            if (Greener(s_fundraisers[i]).s_status() == _status) {
                i_totalFundraisersWithStatus++;
            }
        }

        GreenerModel.FundraiserItem[] memory openFundraisers = new GreenerModel.FundraiserItem[](i_totalFundraisersWithStatus);
        if (i_totalFundraisersWithStatus == 0) {
            return openFundraisers;
        }
        
        for(uint i = 0; i < i_totalFundraisersWithStatus; i++) {
            Greener fundraiser = Greener(s_fundraisers[i]);
    ...
    function donateByAddress(
        ...
        if (_tokenAddress == address(0)) {
            bool success = Greener(_fundraiserAddress).makeDonation{value : _amount}(msg.sender, _amount, _tokenAddress);
            require(success, "Transfer failed.");
        } else {
            // TODO
            revert GreenerFactory__not_implemented();
    ...
    function getMyRecurringPayments() public view returns (GreenerModel.RecurringPaymentDisposition[] memory recurringPayments) {
        recurringPayments = new GreenerModel.RecurringPaymentDisposition[](s_recurringPaymentsByOwner[msg.sender].length);
    ...
    function createRecurringPayment(
        ...
        s_recurringPayments[recurringPaymentId].status = GreenerModel.RecurringPaymentStatus.ACTIVE;
    ...
    function cancelRecurringPayment(
        ...
        if (s_recurringPayments[_id].status == GreenerModel.RecurringPaymentStatus.ACTIVE) {
            s_recurringPayments[_id].status = GreenerModel.RecurringPaymentStatus.CANCELLED;
        }
    }
    ...
    function executeRecurringPayment(uint _id) internal {
        ...
        if (executorBalance >= s_recurringPayments[_id].amount) {
            ...
            Greener fundraiser = Greener(s_recurringPayments[_id].target);
            if (fundraiser.s_status() == GreenerModel.FundraiserStatus.ACTIVE && fundraiser.i_type() == GreenerModel.FundraiserType.RECURRING_DONATION) {
    ...
    function checkUpkeep(bytes memory /* checkData */) public override view returns (
        ...
        for (uint id = 0; id < s_counterRecurringPayments; id++) {
            if (s_recurringPayments[id].status != GreenerModel.RecurringPaymentStatus.ACTIVE) {
                continue;
            }

            if (Greener(s_recurringPayments[id].target).i_type() != GreenerModel.FundraiserType.RECURRING_DONATION) {
                continue;
            }

            if (Greener(s_recurringPayments[id].target).s_status() != GreenerModel.FundraiserStatus.ACTIVE) {
                continue;
            }
